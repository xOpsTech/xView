  ...
  onSearch(searchLogsForm) {
    this.userService.getUserData().subscribe(res => {
      ...
      if (this.search == true) {
        ...
        this.logsearchurl = this.sanitizer.bypassSecurityTrustResourceUrl(config.elasticsearchurl+":5601/app/kibana#/dashboard/33587930-bb30-11e7-82db-656a82ab6465?embed=true&embed=true&_g=(refreshInterval:(display:Off,pause:!f,value:0),time:(from:now-7d,mode:quick,to:now))&_a=(description:'',filters:!(),options:(darkTheme:!f),panels:!((col:1,columns:!(_source),id:'022a9960-bb30-11e7-8c86-41e12d00fa7f',panelIndex:1,row:1,size_x:12,size_y:5,sort:!('@timestamp',desc),type:search)),query:(query_string:(analyze_wildcard:!t,query:'_index:%22logs-" + tenant_id + "-%22*+AND+" + this.searchq + "')),timeRestore:!f,title:'Logs+Dashboard',uiState:(),viewMode:view)");
        this.search = false;
      }
      else {
        this.searchq = encodeURI(searchLogsForm.value.searchq);
