  ...
  onSearch(searchLogsForm) {
    ...
    this.tenant_id = this.userDetails.tenantId;
      ...
      if (this.search == true) {
        ...
        this.logsearchurl = this.sanitizer.bypassSecurityTrustResourceUrl(config.elasticsearchurl+":5601/app/kibana#/dashboard/d7b3ea70-89d2-11e8-af0b-83353d84732f?embed=true&embed=true&_g=(refreshInterval:(display:Off,pause:!f,value:0),time:(from:now-7d,mode:quick,to:now))&_a=(description:'',filters:!(),options:(darkTheme:!f),panels:!((col:1,columns:!(_source),id:'51173b40-89d0-11e8-875c-893ed1ecf74b',panelIndex:1,row:1,size_x:12,size_y:5,sort:!('@timestamp',desc),type:search)),query:(query_string:(analyze_wildcard:!t,query:'_index:%22logs-" + this.tenant_id + "-%22*+AND+" + this.searchq + "')),timeRestore:!f,title:'Logs+Dashboard',uiState:(),viewMode:view)");
        this.search = false;
      }
      else {
        this.searchq = encodeURI(searchLogsForm.value.searchq);
